...
:key: 1) 내 아이디와 비밀번호를 입력하여 로그인
:ballot_box_with_check: 2) 내 ID에 해당하는 전체 다이어리 정보를 확인한다
:couplekiss: 3) 나와 커플인 이용자의 다이어리 정보를 확인한다
:pencil: 4) 다이어리 쓰기 
:calendar: 5) 특정 날짜에 해당하는 다이어리 수정
:x: 6) 특정 날짜에 해당하는 다이어리 삭제
:speech_balloon: 7) 오늘 내 기분과 같은 기분인 사람들의 한줄 코멘트 보기
:bar_chart: 8) 나의 감정상태 측정  - ① 자신이 지금까지 기록했던 기분점수 평균과 
...
:sleeping: 9) 나의 수면상태 측정  - ① 지금까지 기록했던 수면시간 평균과 
			② 최근 7일간의 수면시간 평균 비교 
					id님, 
...
:busts_in_silhouette: 2) 감정 테이블에 감정 추가/삭제 (향후 Update용)
:droplet: 3) 날씨 테이블에 날씨 추가/삭제 (향후 Update용)
